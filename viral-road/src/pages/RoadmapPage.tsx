...
import { 
  Loader2, X, FileText, Instagram, Youtube, Music2, Zap, Globe, Layout, Calendar, Edit3, CheckCircle2, ChevronLeft, ChevronRight, Trash2, Tag, CheckSquare, Clock, Plus, Circle, CalendarDays, AlertTriangle
} from 'lucide-react';
...
const RoadmapPage: React.FC<RoadmapPageProps> = ({ user }) => {
  ...
  const handleExecuteDelete = async () => {
    ...
    try {
      const { error } = await supabase.from("content_blueprints").delete().eq("id", itemToDelete).eq("user_id", user.id);
  ...
  return (
    <div className="p-4 md:p-10 space-y-8 pb-20 animate-in fade-in h-full flex flex-col relative">
                                  ...
                                  <Draggable key={item.id} draggableId={item.id} index={index}>
                                    {(provided, snapshot) => (
                                      <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} onClick={() => setSelectedContent(item)} style={{ ...provided.draggableProps.style }} className="mb-3">
                                        <div className={`bg-white dark:bg-zinc-950 p-5 rounded-[1.8rem] border hover:border-yellow-400 transition-all cursor-pointer group shadow-sm relative overflow-hidden ${snapshot.isDragging ? 'shadow-2xl ring-2 ring-yellow-400 rotate-2 z-50 scale-105' : 'border-zinc-100 dark:border-zinc-800'}`}>
      ...
      <AnimatePresence>
        {selectedContent && (
          <ContentModal content={selectedContent} onClose={() => { setSelectedContent(null); fetchData(); }} onDelete={() => confirmDelete(selectedContent.id)} />
        )}
      </AnimatePresence>
...
const ContentModal = ({ content, onClose, onDelete }: any) => {
  ...
  useEffect(() => {
    ...
    // Reduced debounce to 500ms for faster feedback
