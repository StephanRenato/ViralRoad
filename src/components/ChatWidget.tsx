...
export const ChatWidget: React.FC<{ user: User }> = ({ user }) => {
  ...
  const handleSendMessage = async () => {
    ...
    const contents = [...messages, { role: 'user', text: userMessage }].map(m => ({
      role: m.role,
      parts: [{ text: m.text }]
    }));
    ...
    try {
      ...
      const aiText = data.text || (typeof data === 'string' ? data : JSON.stringify(data));
      setMessages(prev => [...prev, { role: 'model', text: aiText }]);

    } catch (proxyError) {
      console.warn("Proxy Chat falhou, tentando fallback local...", proxyError);
      ...
      try {
        ...
        setMessages(prev => [...prev, { role: 'model', text: chatResponse.text || "Sem resposta." }]);
        
      } catch(finalError) {
         console.error("Erro no chat:", finalError);
         setMessages(prev => [...prev, { role: 'model', text: "Ocorreu um erro de conexão. Verifique sua internet." }]);
      }
    } finally {
      setIsTyping(false);
    }
